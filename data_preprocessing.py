...
def make_spect_f0(config):
    ...
    spk_meta = pickle.load(open('meta.pkl', "rb"))
    ...
    for spk_dir in sorted(spk_dir_list):
        ...
        for filename in sorted(file_list):
            ...
            if np.isnan(f0).any():
                print(f'Warning: NaN values encountered in F0 for {filename}')
                continue
            wavs.append(wav)
            f0s.append(f0)
            sps.append(sp)
...
def make_metadata(config):
    ...
    for spk_dir in sorted(spk_dir_list):
        if spk_dir not in spk_meta:
            print(f'Warning: {spk_dir} not in speaker metadata; skipping')
            continue
