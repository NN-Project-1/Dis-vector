...
class Encoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.interp = InterpLnr(config)
        #self.embedding_layer = nn.Embedding(self.emb_size, 80)
    ...
    def forward(self, x_f0, rr=True):  
        ...
        linear_layer = nn.Linear(flattened_codes_x.size(1), embedding_dim)
        embedding = linear_layer(flattened_codes_x)
        
            # Output: [batch_size, embedding_dim]
        return codes_x,embedding

class Decoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dim_neck = config.dim_neck   # 32
        self.dim_neck_2 = config.dim_neck_2  # 32
        self.dim_emb = config.dim_spk_emb   # 82
        self.dim_freq = config.dim_freq    # 80
        self.dim_neck_3 = config.dim_neck_3 # 32
        
        self.lstm = nn.LSTM(64,512, 3, batch_first=True, bidirectional=True)  # (32*2+32*2+32*2+82,512,3) (274,512,3)
        
        self.linear_projection = nn.Linear(512 * 2, self.dim_freq)  # (512 * 2, 80 * 2)
...
class Generator_3(nn.Module):
    ...
    def forward(self, x_f0, rr=True):
        ...
        # Embeddings
        codes_x,emb = self.encoder_1(x_1, rr)
        code_exp_1 = codes_x.repeat_interleave(1, dim=1)
        # Decode to obtain mel outputs
        mel_outputs = self.decoder(code_exp_1)
        return mel_outputs
    ...
    def content_pitch(self, x_f0, rr=True):
        ...
        return code_exp_1,emb

    def decode(self, code_exp_1):
        code_exp_1,emb = code_exp_1
        encoder_outputs = code_exp_1
